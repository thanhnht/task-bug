...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        html, body { height: 100%; background: var(--bg-0); color: var(--text-1); font-family: var(--font-body); font-size: 14px; }
        ...
        .nav-item:hover { background: var(--bg-2); color: var(--text-1); border-left-color: var(--border-lit); }
        ...
        .user-role.employee { color: var(--blue); }
        ...
        .page-header p { margin-top: 4px; font-size: 13.5px; color: var(--text-2); }
        ...
        .btn-ghost:hover { background: var(--bg-2); color: var(--text-1); border-color: var(--border-lit); }
        ...
        .form-error { margin-top: 5px; font-size: 12px; color: var(--red); display: flex; align-items: center; gap: 4px; }
        ...
        .alert-info    { background: rgba(59,130,246,.08); border-color: rgba(59,130,246,.3); color: #93c5fd; }
        ...
        .badge-employee { background: rgba(59,130,246,.15); color: var(--blue); }
        .badge-active   { background: rgba(34,197,94,.15);  color: var(--green); }
        .badge-inactive { background: rgba(239,68,68,.12);  color: var(--red); }
        .badge-locked   { background: rgba(234,179,8,.12);  color: var(--yellow); }
        ...
        tbody td { padding: 11px 14px; font-size: 13.5px; color: var(--text-1); vertical-align: middle; }
        ...
        .stat-label { font-size: 11px; font-family: var(--font-mono); text-transform: uppercase; letter-spacing: .1em; color: var(--text-3); margin-bottom: 8px; }
        .stat-value { font-family: var(--font-mono); font-size: 28px; font-weight: 700; color: var(--text-1); }
        .stat-sub { font-size: 12px; color: var(--text-2); margin-top: 4px; }

        /* ── Toast (flash) ─────────────────────────────────────────────────── */
        .toast-area { position: fixed; top: 70px; right: 24px; z-index: 999; display: flex; flex-direction: column; gap: 8px; }
        .toast {
            background: var(--bg-2);
            border: 1px solid var(--border-lit);
            border-radius: var(--radius);
    ...
    <aside class="sidebar">
        <a href="{{ route('employee.dashboard') }}" class="sidebar-logo">
            <div class="sidebar-logo-icon">
                <svg viewBox="0 0 16 16"><path d="M2 3h12v2H2V3zm0 4h8v2H2V7zm0 4h10v2H2v-2z"/></svg>
            </div>
        ...
        <nav class="sidebar-nav">
            ...
            <a href="#" class="nav-item {{ request()->routeIs('stories.*') ? 'active' : '' }}">
                <svg viewBox="0 0 16 16" fill="currentColor"><path d="M2 2h12v1H2V2zm0 3h12v1H2V5zm0 3h8v1H2V8zm0 3h10v1H2v-1z"/></svg>
                Stories
                <span class="nav-badge">4</span>
            </a>

            <a href="#" class="nav-item {{ request()->routeIs('bugs.*') ? 'active' : '' }}">
                <svg viewBox="0 0 16 16" fill="currentColor"><path d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zM7 4h2v5H7V4zm0 6h2v2H7v-2z"/></svg>
                Bug Reports
                <span class="nav-badge yellow">7</span>
            </a>

            <a href="#" class="nav-item {{ request()->routeIs('subtasks.*') ? 'active' : '' }}">
                <svg viewBox="0 0 16 16" fill="currentColor"><path d="M3 3h10v1H3V3zm2 3h8v1H5V6zm2 3h6v1H7V9z"/></svg>
                Subtasks
            </a>
            ...
            <a href="{{ route('auth.change-password') }}"
               class="nav-item {{ request()->routeIs('auth.change-password') ? 'active' : '' }}">
                <svg viewBox="0 0 16 16" fill="currentColor"><path d="M8 1a4 4 0 0 1 4 4v1h1a1 1 0 0 1 1 1v7a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V7a1 1 0 0 1 1-1h1V5a4 4 0 0 1 4-4zm0 2a2 2 0 0 0-2 2v1h4V5a2 2 0 0 0-2-2z"/></svg>
                Đổi mật khẩu
            </a>
        </nav>
        ...
        <div class="sidebar-user">
            ...
            <form method="POST" action="{{ route('auth.logout') }}">
                @csrf
                <button type="submit" title="Đăng xuất" style="background:none;border:none;cursor:pointer;color:var(--text-3);padding:4px;">
                    <svg viewBox="0 0 16 16" fill="currentColor" style="width:16px;height:16px;">
                        <path d="M6 2h4v2H6V2zm-3 1h2v10H3a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1zm10 0a1 1 0 0 1 1 1v8a1 1 0 0 1-1 1h-2V3h2zM9 7.5l-3 3V9H3V7h3V5.5l3 2z"/>
                    </svg>
                </button>
            </form>
        </div>
        ...
        <div class="toast-area" id="toastArea">
            @if(session('success'))
                <div class="toast success">
                    <svg viewBox="0 0 16 16" fill="currentColor"><path d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zM6.5 10.8 3.7 8l1-1 1.8 1.8 4-4 1 1-5 5z"/></svg>
                    <span>{!! session('success') !!}</span>
                </div>
            @endif
            @if(session('warning'))
                <div class="toast warning">
                    <svg viewBox="0 0 16 16" fill="currentColor"><path d="M8 1L1 14h14L8 1zm0 3 5 9H3l5-9zm-1 3v3h2V7H7zm0 4v2h2v-2H7z"/></svg>
